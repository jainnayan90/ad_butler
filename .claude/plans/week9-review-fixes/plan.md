...
- [ ] [P1-T1][ecto] Add `Chat.flip_streaming_messages_to_error/1` to
...
- [ ] [P1-T2] Add `Chat.get_session_user_id!/1` to `chat.ex` (or extend
...
- [ ] [P1-T4] Tests for `Chat.flip_streaming_messages_to_error/1` in
...
- [ ] [P1-T5] Update `Chat.Server` test for `terminate/2` to use the new
  context fn (no longer asserts on internal `Repo.update`).
- [ ] [P1-T6] **Verify**: phase check loop green; chat tests still pass.
...
- [ ] [P2-T1] In `Chat.Server.react_loop/3`, mint a `request_id` UUID
  ...
  ```
- [ ] [P2-T2] Persist the assistant `chat_messages.request_id` column
...
- [ ] [P2-T3] Update [e2e_test.exs](test/ad_butler/chat/e2e_test.exs):
...
- [ ] [P2-T5] **Verify**: phase loop + e2e test (`mix test --include
  integration`).
...
- [ ] [P3-T1] [application.ex:152](lib/ad_butler/application.ex#L152) —
  drop `inspect/1` wrapper on `reason` in the Oban exception handler.
  `reason: reason` (B3).
- [ ] [P3-T2] [get_insights_series.ex](lib/ad_butler/chat/tools/get_insights_series.ex)
...
- [ ] [P3-T4] H3 — rename `Chat.ensure_server!/1` → `Chat.ensure_server/1`:
...
- [ ] [P4-T1] H2 + W8 — replace ordering `:timer.sleep` calls with
...
- [ ] [P4-T2] W1 — `Chat.Telemetry.attach()` cleanup:
...
- [ ] [P4-T3] W5 — fix `stop_supervised!(Server)` →
...
- [ ] [P4-T7] W4 — add a tenant-isolation test for `list_messages/2`:
  ...
  - Two sessions, two users. `list_messages(session_a.id)` returns
    ...
    `get_session!/2` first".
- [ ] [P4-T8] W13 — extend `paginate_messages/2` test in
...
- [ ] [P4-T9] W14 — drop the duplicate `meta_connection` in
...
- [ ] [P4-T10] **Verify**: phase loop + chat tests; spot-check no
...
- [ ] [P5-T1] W9 — add anti-prompt-injection paragraph to
...
- [ ] [P5-T3] W11 — add `# TODO(W11): bulk fetch via single Analytics
...
- [ ] [P5-T4] W12 — extract shared helpers to
...
- [ ] [P5-T5] **Verify**: full check loop + full test suite.

---

## Acceptance

- [ ] All 17 distinct fixes landed (3 BLOCKERS + 3 HIGH + 11 WARNINGS
  after dedup).
- [ ] No `Repo.` calls in `lib/ad_butler/chat/server.ex`,
  `lib/ad_butler/chat/agent.ex`, or `lib/ad_butler/chat/telemetry.ex`.
  (Tools already enforced via `mix check.tools_no_repo`.)
- [ ] `Chat.Server.react_loop/3` calls `Telemetry.set_context/1` before
  every `llm_client().stream/2` and `clear_context/0` after (try/after).
- [ ] No `String.to_existing_atom/1` on raw LLM-supplied strings; all
  go through guarded `_to_atom/1` mappers.
- [ ] No `inspect/1` wrappers in `Logger.error`/`Logger.warning`
  metadata in `lib/ad_butler/`.
- [ ] No `:timer.sleep` in chat tests except the one documented
  hibernate-test exception.
- [ ] `Chat.Telemetry.attach/0` is detached on test exit
  (telemetry_test, e2e_test).
- [ ] `Chat.ensure_server/1` (no bang) takes `(user_id, session_id)`
  and re-validates.
- [ ] `Chat.send_message/3` has a tenant-isolation test.
- [ ] System prompt includes the anti-injection paragraph; compile-time
